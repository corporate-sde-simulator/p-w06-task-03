...
> **Welcome to Product-Track Week 6, Task 3!**
...
| `src/evictionPolicy.py` | Main source code - **has bugs to fix** |
| `src/lruCache.py` | Supporting code - **may also have bugs** |
| `tests/test_lruCache.py` | Test file - **add your tests here** |
...
### Bug #1 - in `src/lruCache.py` (around line 65)
**What's wrong:** last=True removes the MOST recently used item (end of OrderedDict).

**How to find it:** Open `src/lruCache.py` and look for the `BUG:` comment near line 65. Read the comment carefully - it describes exactly what's broken.
...
### Bug #2 - in `src/lruCache.py` (around line 77)
**What's wrong:** Comparison is inverted — returns True (expired) when time is BEFORE expiry,

**How to find it:** Open `src/lruCache.py` and look for the `BUG:` comment near line 77. Read the comment carefully - it describes exactly what's broken.
